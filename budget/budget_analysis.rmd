...
engine_actual<-engine_just_actual %>%
  mutate(
    stagename=factor_wrap(stage,nwrap=12),
    stage = recode(
      stage,
...
levels(engine_actual$stagename)=
  list("(6.1) Basic\nResearch"="(6.1) Basic\nResearch",
       "(6.2) Applied\nResearch"="(6.2) Applied\nResearch",                             
 "(6.3) Advanced\nTechnology\nDevelopment"="(6.3) Advanced\nTechnology\nDevelopment",
"(6.4) Advanced\nComponent\nDevelopment &\nPrototypes"="(6.4) Advanced\nComponent\nDevelopment &\nPrototypes",
"(6.5) System\nDevelopment &\nDemonstration"="(6.5) System\nDevelopment &\nDemonstration",          
"(6.7) Operational\nSystems\nDevelopment"=c("(6.7) Operational\nSystems Development",
                                           "(6.7) Operational\nSystems\nDevelopment"
)
)
...
engine_budget_organization_future<-engine_budget_organization_future%>%
  left_join(rdtedept)
...
(
  plot_ebf <- ggplot(engine_budget_organization_future) +
    geom_line(
      aes(
        x = FY,
        y = Amount_OMB23_GDP21/ServiceTOA_OMB23_GDP21,
        group = PByear,
        alpha = PByear
      ),
      color = "#554449",
...
engine_actual_FY <- engine_actual %>%
  ...
  dplyr::summarize(Amount_Then_Year = sum(Amount_Then_Year, na.rm = TRUE),
                   Amount_OMB23_GDP21 = sum(Amount_OMB23_GDP21, na.rm = TRUE))%>%
  left_join(rdte %>%select(FY,TOA_OMB23_GDP21),by=c("FY"="FY"))
...
(
  plot_eba <- build_plot(engine_actual,
    ...
    scale_y_continuous(label = unit_format(unit = "M", scale = 1e-6)) +
    xlab("Fiscal Year") + 
    ylab("Budget (Constant 2021 $)") +geom_vline(
      xintercept = 2021,
      color = "#554449",
      alpha = .5,
      linetype = "solid"
...
(
  plot_eorgact <- build_plot(engine_actual,
                         ...
                         y_var="Amount_OMB23_GDP21",
                         facet="Organization",
                         # color_var = "Organization",
                         # labels_and_colors=budget_lc,
                         # column_key = budget_ck,
                         format=TRUE
                         ) +
    labs(main="DoD RDT&E Aircraft Engine Spending Projections",
         caption="Source: CSIS analysis based on RDT&E budget documents (PB 2000–21),\nwhich can be found at https://comptroller.defense.gov/Budget-Materials/. ") +
    # scale_y_continuous(label = unit_format(unit = "M", scale = 1e-6)) +
    xlab("Fiscal Year") + 
    ylab("Budget (Constant 2021 $)") +geom_vline(
      xintercept = 2021,
...
engine_actual_FY_org <- engine_actual %>%
  # filter(Amount_Then_Year != 0) %>%
  group_by(FY,Organization) %>%
  dplyr::summarize(Amount_Then_Year = sum(Amount_Then_Year, na.rm = TRUE),
                   Amount_OMB23_GDP21 = sum(Amount_OMB23_GDP21, na.rm = TRUE))%>%
  left_join(rdtedept %>%select(FY,Organization,ServiceTOA_OMB23_GDP21),by=c("FY","Organization"))%>%
  mutate(pEngine=Amount_OMB23_GDP21/ServiceTOA_OMB23_GDP21)
(
 plot_peorgact <- build_plot(engine_actual_FY_org,
                         chart_geom="Line Chart",
                         share=FALSE,
                         x_var="FY",
                         y_var="pEngine",
    ...
    scale_y_continuous(label = label_percent()) +
    xlab("Fiscal Year") + 
    ylab("Engines as a Percent of\nDepartment RDT&E\nTotal Obligation Authority") +geom_vline(
      xintercept = 2021,
      color = "#554449",
      alpha = .5,
      linetype = "solid"
...
(
both<-gridExtra::grid.arrange(plot_eorgact+labs(title=NULL,x=NULL,caption=NULL),
                              plot_peorgact+labs(title=NULL))
)


ggsave(
  "charts/actual_amount_service_both.svg",
  both,
  # device = "svg",
  width = 6.5,
  height = 4.5,
...
ggsave(
  "charts/actual_amount_service_both.emf",
  both,
  # device = "svg",
  width = 6.5,
  height = 4.5,
...
ggsave(
  "charts/actual_amount_service_both.png",
  both,
  # device = "svg",
  width = 6.5,
  height = 4.5,
...
budget_lc<-prepare_labels_and_colors(engine_actual_1,path=local_path)
budget_ck<-get_column_key(engine_actual_1,path=local_path)
(
  facet <-
     build_plot(engine_actual_1,
                           chart_geom="Bar Chart",
                           share=FALSE,
                           x_var="FY",
                           y_var="Amount_OMB23_GDP21",
                           facet_var="topProjectText",
                color_var = "Organization",
                labels_and_colors = budget_lc,
                column_key = budget_ck,
                           format=TRUE,
    ...
    labs(main="DoD RDT&E Aircraft Engine Spending By Project",
         caption="Source: CSIS analysis based on RDT&E budget documents (PB 2000–21),\nwhich can be found at https://comptroller.defense.gov/Budget-Materials/. ") +
    xlab("Fiscal Year") +
    ylab("Budget (Constant 2021 $)") + 
    geom_vline(
...
ggsave(
  "charts/actual_Amount_project.emf",
  facet+theme(legend.position="bottom")+labs(title=NULL),
  width = 6.5,
  height = 7,
  units = "in"
...
ggsave(
  "charts/actual_Amount_project.png",
  facet+theme(legend.position="bottom")+labs(title=NULL),
  width = 6.5,
  height = 7,
  units = "in",
...
empty_stage<-expand.grid(
  stagename = levels(engine_actual$stagename),
  Organization = levels(factor(engine_actual$Organization)),
  FY=seq(from=1998,to=2023))
empty_stage$Amount_OMB23_GDP21<-0
  ...
  (
    plot_eorgstageact <- build_plot(rbind(engine_actual,
                                          empty_stage),
                           chart_geom="Line Chart",
                           share=FALSE,
                           x_var="FY",
        ...
        labs(main="DoD RDT&E Aircraft Engine Spending By Stage",
         caption="Source: CSIS analysis based on RDT&E budget documents (PB 2000–21),\nwhich can be found at https://comptroller.defense.gov/Budget-Materials/. ") +
      # scale_y_continuous(label = unit_format(unit = "M", scale = 1e-6)) +
         scale_x_continuous(
        breaks = seq(2000, 2027, by = 10),
  ...
  ggsave600dpi(
    "charts/actual_amount_service_stage_nocrosstab.png",
    plot_eorgstageact+theme(legend.position="non")+labs(title=NULL),
    width = 7.5,
    height = 4,
    units = "in",
...
RnDcontract<-engine_contracts %>% filter(SimpleArea.AETP=="R&D and AETP" &
                                           SubCustomer %in% c("Army","Navy","Air Force")) %>%
  group_by(Fiscal_Year,ProductServiceOrRnDarea.project,SubCustomer)%>%
  summarise(Amount_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21,na.rm=TRUE))
colnames(RnDcontract)[colnames(RnDcontract)=="SubCustomer"]<-"Organization"
colnames(RnDcontract)[colnames(RnDcontract)=="Fiscal_Year"]<-"FY"
colnames(RnDcontract)[colnames(RnDcontract)=="ProductServiceOrRnDarea.project"]<-"stagename"


stage<-rbind(engine_actual %>% mutate(LineType="Budget"),
                                          empty_stage%>% mutate(LineType="Budget"),
                                          RnDcontract%>%mutate(LineType="Contract"),
                                          empty_stage%>% mutate(LineType="Contract"))
stage$stagename<-factor(stage$stagename)
  summary(stage$stagename)

 levels(stage$stagename)<-list(
      '(6.1) Basic\nResearch'=
        c('(6.1) Basic\nResearch',
          "Basic Research (6.1)" ),
      '(6.2) Applied\nResearch'=
  ...
  (
    plot_eorgstageactk <- build_plot(stage,
                           chart_geom="Line Chart",
                           share=FALSE,
                           x_var="FY",
                           y_var="Amount_OMB23_GDP21",
