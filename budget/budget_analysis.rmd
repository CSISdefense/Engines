...
engine_actual<-engine_just_actual %>%
  ...
  group_by(FY, Organization, stage, ProjectName) %>%
...
engine_budget_project_future <- engine_budget %>%
  mutate(FY = as.numeric(FY)) %>%
  group_by(PByear, FY, ProjectName) %>%
  filter(Amount_Then_Year != 0) %>%
  dplyr::summarize(Amount_Then_Year = sum(Amount_Then_Year, na.rm = TRUE),
                   Amount_OMB23_GDP21 = sum(Amount_OMB23_GDP21, na.rm = TRUE))
...
(
  facet <-
    ggplot() + geom_area(aes(y = Amount_OMB23_GDP21, x = FY),
...
write.csv(
  engine_actual %>%
    group_by(FY, Organization, ProjectName) %>%
    summarise(Amount_Then_Year=sum(Amount_Then_Year,na.rm=TRUE)) %>%
    pivot_wider(
    names_from=FY,values_from=Amount_Then_Year) %>%
    arrange(Organization,ProjectName),
    file="charts/actual_Amount_org_project.csv",
  na = "",
  row.names = FALSE
  )
