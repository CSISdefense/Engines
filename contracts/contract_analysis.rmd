...
local_path<-"https://raw.githubusercontent.com/CSISdefense/Lookup-Tables/master/style/"
...
Top_Vendor<-
engine_contracts %>% group_by(AllContractor)%>%
  summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21)
            # Action_Obligation_2020=sum(ifelse(Fiscal_Year==2020,Action_Obligation_OMB23_GDP21,0))
            )%>%
              group_by () %>%
...
Top_Vendor %>% dplyr::filter( rank_total<=5)

engine_contracts$TopVendors<-engine_contracts$AllContractor
engine_contracts$TopVendors[engine_contracts$TopVendors %in% 
                              Top_Vendor$AllContractor[Top_Vendor$rank_total>5]]<-"All Other Vendors"
...
### Top  5 graph
```{r TopVendor}
engine_contracts$TopVendorsPretty<-factor(engine_contracts$TopVendors)
levels(engine_contracts$TopVendorsPretty)<-
  list("Pratt & Whitney"="UNITED TECH",
       "General Electric"="GENERAL ELECTRIC",
      "Rolls Royce"="ROLLS ROYCE",
       "Honeywell"="HONEYWELL",
       "Kelly Aviation Center (JV)"="KELLY AVIATION CENTER [Joint Venture Lockheed Martin/Rolls Royce]",
      "All Other Vendors"="All Other Vendors")

(
  TopVendors <- engine_contracts %>%
    filter(TopVendors != "Unlabeled") %>%
    group_by(Fiscal_Year, TopVendorsPretty) %>%
    dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
    ggplot() +
    geom_area(aes(y = Action_Obligation_OMB23_GDP21, x = Fiscal_Year), alpha = .9, stat = "identity") +
    facet_wrap(~ TopVendorsPretty, nrow = 2) +
    chart_theme +
    ggtitle("DoD Aircraft Engine Contract Obligations by Vendor Size") +
    xlab("Fiscal Year") +
    ylab("Constant 2021 $") +
...
(
  Top3 <- engine_contracts %>%
    filter(TopVendorsPretty %in% c("Pratt & Whitney","General Electric","Rolls Royce")) %>%
    group_by(Fiscal_Year, TopVendorsPretty) %>%
    dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
    ggplot() +
    geom_area(aes(y = Action_Obligation_OMB23_GDP21, x = Fiscal_Year), alpha = .9, stat = "identity") +
    facet_wrap(~ TopVendorsPretty, nrow = 1) +
    chart_theme +
    ggtitle("DoD Aircraft Engine Contract Obligations by Vendor Size") +
    xlab("Fiscal Year") +
    ylab("Constant 2021 $") +
...
ggsave(
  "charts/amount_Top3.svg",
  Top3+labs(title=NULL),
  device = "svg",
  width = 8,
  height = 3,
  units = "in"
...
(
  PW <- engine_contracts %>%
    filter(TopVendorsPretty == "Pratt & Whitney") %>%
    group_by(Fiscal_Year, TopVendorsPretty) %>%
    dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
    ggplot() +
    geom_area(aes(y = Action_Obligation_OMB23_GDP21, x = Fiscal_Year), alpha = .9, stat = "identity") +
...
(
  GE <- engine_contracts %>%
    filter(TopVendorsPretty == "General Electric") %>%
    group_by(Fiscal_Year, TopVendorsPretty) %>%
    dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
    ggplot() +
    geom_area(aes(y = Action_Obligation_OMB23_GDP21, x = Fiscal_Year), alpha = .9, stat = "identity") +
...
(
  RR <- engine_contracts %>%
    filter(TopVendorsPretty == "Rolls Royce") %>%
    group_by(Fiscal_Year, TopVendorsPretty) %>%
    dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
    ggplot() +
    geom_area(aes(y = Action_Obligation_OMB23_GDP21, x = Fiscal_Year), alpha = .9, stat = "identity") +
...
write.csv(file="charts//amount_top_vendor.csv",row.names = FALSE, na = "",
          TopVendors$data%>% mutate(Fiscal_Year)%>%#=lubridate::year(dFYear)
            pivot_wider(id_cols=c(TopVendors),
...
(
  RnDphase<-build_plot(
    ...
    y_var="Action_Obligation_OMB23_GDP21", #VAR.y.variable
    color_var="ProductServiceOrRnDarea.project", #color_var
...
output_TY<-engine_contracts %>% filter(SimpleArea=="R&D") %>%
  format_data_for_plot(fy_var="Fiscal_Year",y_var="Action_Obligation_Then_Year",color_var = "ProductServiceOrRnDarea.project",
                       labels_and_colors=labels_and_colors) %>%
  pivot_wider(id_cols=c(ProductServiceOrRnDarea.project),
              names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%
  arrange(ProductServiceOrRnDarea.project)
...
(
  ColorOfMoney<-build_plot(
    data=engine_contracts %>% filter(!is.na(SimpleArea) & 
                                       Fiscal_Year>=2011 & Fiscal_Year<=2017),
    chart_geom = "Bar Chart",
    share = FALSE,
    labels_and_colors=labels_and_colors,
    # NA, #VAR.ncol
    ...
    y_var="Action_Obligation_OMB23_GDP21", #VAR.y.variable
    color_var="SimpleArea.engines", #color_var
    facet_var="ColorOfMoney", #facet_var
    column_key=column_key,
    format=TRUE,
    ytextposition=FALSE
  )+
    ...
    theme(legend.position = "bottom")+
    labs(y="Obligations (Constant 2021 $s)")+
    facet_wrap(ColorOfMoney~.,nrow=2)
)



(
  ColorOfMoneySubCust<-build_plot(
    data=engine_contracts %>% filter(!is.na(SimpleArea) & 
                                       Fiscal_Year>=2011 & Fiscal_Year<=2017),
    chart_geom = "Bar Chart",
    share = FALSE,
    ...
    x_var="dFYear", #x_var
    y_var="Action_Obligation_OMB23_GDP21", #VAR.y.variable
    color_var="SubCustomer.engines", #color_var
    facet_var="ColorOfMoney", #facet_var
...
ggsave600dpi(ColorOfMoney,file="charts/ColorOfMoney.png",width=6.5,height=2.75,size = 11)
ggsave600dpi(ColorOfMoney,file="charts/ColorOfMoney.svg",width=6.5,height=2.75,size = 11)
ggsave600dpi(ColorOfMoneySubCust,file="charts/ColorOfMoneySubCust.png",width=5,height=6,size = 11)
ggsave600dpi(ColorOfMoneySubCust,file="charts/ColorOfMoneySubCust.svg",width=3.25,height=6,size = 11)

write.csv(file="charts//ColorOfMoney.csv",row.names = FALSE, na = "",
          ColorOfMoney$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            arrange(Fiscal_Year) %>%
            pivot_wider(id_cols=c(ColorOfMoney,SimpleArea.engines),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21)%>%
            arrange(ColorOfMoney,SimpleArea.engines))

write.csv(file="charts//ColorOfMoneySubCust.csv",row.names = FALSE, na = "",
          ColorOfMoneySubCust$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            arrange(Fiscal_Year) %>%
            pivot_wider(id_cols=c(ColorOfMoney,SubCustomer.engines),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21)%>%
            arrange(ColorOfMoney,SubCustomer.engines))
...
ggsave("Charts//subcustomer_PSR.png",
             SubCustomerPSR+labs(caption="Source: FPDS and CSIS analysis."), 
             width=6.5, height= 6, units="in", dpi=600 #size=12, lineheight=1
             )
...
summary(SubCustomerPSR$data$SubCustomer.engines)
summary(SubCustomerPSR$data$SimpleArea.engines)

summary(factor(engine_contracts$SubCustomer.engines))
summary(factor(engine_contracts$SimpleArea.engines))
# View(engine_contracts %>% filter(SubCustomer.engines!="F-35 JPO" & SimpleArea.engines=="JSF (F-35)"))
write.csv(row.names = FALSE,engine_contracts %>% filter(SubCustomer.engines!="F-35 MDAP" & SimpleArea.engines=="JSF (F-35)"),
          file = "charts/f35conflict.csv")
```
...
comparison_contracts <- comparison_contracts %>%
  left_join(dyear, by = c("Fiscal_Year", "SubCustomer", "SimpleArea", "SimpleArea.AETP","type","dFYear"))# %>%
...
dyear <- comparison_contracts %>%
  ...
  group_by()%>%
  select(-Action_Obligation_OMB23_GDP21.lagged,-Fiscal_Year,-dFYear)

#Add 2000
sumcheck<-sum(comparison_contracts$Action_Obligation_OMB23_GDP21,na.rm=TRUE)
comparison_contracts <- comparison_contracts %>%
  left_join(dyear, by = c( "SubCustomer", "SimpleArea", "SimpleArea.AETP","type")) %>%
...
ggsave(file="charts/SimpleAreaBoth.svg",
       both,
       width=6.5,height=4.5,dpi=600)
ggsave(file="charts/SimpleAreaBoth.png",
       both,
       width=6.5,height=4.5)
...
write.csv(file="charts//pSimpleArea.csv",row.names = FALSE, na = "",
          pSA$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            ...
            arrange(desc(SimpleArea.AETP),MetricName)) 
