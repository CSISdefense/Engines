...
labels_and_colors<-prepare_labels_and_colors(engine_contracts %>% select(-PricingMechanism))#, path=local_path)
...
write.csv(file="data\\service_invest.csv",engine_contracts %>%
  select(SimpleArea,ClaimantProgramCode,PlatformPortfolio,ProductOrServiceCode,ProductOrServiceCodeText,ProductServiceOrRnDarea,Action_Obligation_OMB23_GDP21,SubCustomer.JPO,Contracting_Agency_ID,ContractingOfficeID,fundingrequestingagencyid,fundingrequestingofficeid,ProjectID,Project.Name) %>%
    group_by(SimpleArea,ClaimantProgramCode,PlatformPortfolio,ProductOrServiceCode,ProductOrServiceCodeText,ProductServiceOrRnDarea,SubCustomer.JPO,Contracting_Agency_ID,ContractingOfficeID,fundingrequestingagencyid,fundingrequestingofficeid,ProjectID,Project.Name) %>%
...
write.csv(file="data\\service_invest_year.csv",engine_contracts %>%
  select(SimpleArea,ClaimantProgramCode,PlatformPortfolio,ProductOrServiceCode,ProductOrServiceCodeText,ProductServiceOrRnDarea,Action_Obligation_OMB23_GDP21,SubCustomer.JPO,Contracting_Agency_ID,ContractingOfficeID,fundingrequestingagencyid,fundingrequestingofficeid,ProjectID,Project.Name,Fiscal_Year) %>%
    group_by(SimpleArea,ClaimantProgramCode,PlatformPortfolio,ProductOrServiceCode,ProductOrServiceCodeText,ProductServiceOrRnDarea,SubCustomer.JPO,Contracting_Agency_ID,ContractingOfficeID,fundingrequestingagencyid,fundingrequestingofficeid,ProjectID,Project.Name,Fiscal_Year) %>%
    summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21,na.rm=TRUE)),row.names = FALSE)
```
...
(both<-gridExtra::grid.arrange(CustCompMulti+labs(caption=NULL,x=NULL)+facet_wrap(SubCustomer.JPO~.,nrow=1)+
                                 theme(legend.position="none")+
                          labs(y="Obligations\n(Constant 2021 $s)")+
            scale_x_date(NULL,labels = scales::label_date("'%y"),date_breaks = "5 years"),
                         CustCompMultiShare+
         facet_wrap(SubCustomer.JPO~.,nrow=1)+
...
write.csv(file="charts//EngineCompSubCust.csv",row.names = FALSE, na = "",
          CustCompMulti$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(SubCustomer.JPO,Competition.multisum),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21)%>%
            arrange(SubCustomer.JPO,Competition.multisum))


write.csv(file="charts//EngineCompSubCust_cur.csv",row.names = FALSE, na = "",
          engine_contracts %>% group_by(SubCustomer.JPO,Competition.multisum,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SubCustomer.JPO,Competition.multisum),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%arrange(SubCustomer.JPO,Competition.multisum))
...
write.csv(file="charts//SimpleArea.csv",row.names = FALSE, na = "",
          SimpleArea$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(SimpleArea.AETP),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21))
...
## Build Plot
```{r SubCustomerPSR}
summary(factor(engine_contracts$SubCustomer.JPO))
(
SubCustomerPSR<-build_plot(
  ...
  x_var="dFYear", #x_var
  y_var="Action_Obligation_OMB23_GDP21", #VAR.y.variable
  color_var="SimpleArea.AETP", #facet_var
 ...
 #             )+labs(title=NULL,
  ...
  labs(y="Obligations (Constant 2021 $s)")+
  scale_y_continuous(expand=expansion(add=c(0,1.5e9)),label = scales::unit_format(unit = "M", scale = 1e-6))+
    facet_grid(SimpleArea.AETP~SubCustomer.JPO,scales="free_y",space="free_y")
  # facet_grid(SimpleArea.AETP~SubCustomer.JPO, scales = "free_y",space="free_y")
)
...
#              )
ggsave600dpi("Charts//subcustomer_PSR.svg", SubCustomerPSR, 
             width=6.5, height= 6, units="in",size=12,lineheight = 1.2
             )

write.csv(file="charts//subcustomer_PSR.csv",row.names = FALSE, na = "",
          SubCustomerPSR$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(SubCustomer.JPO,SimpleArea.AETP),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21)%>%
            arrange(SubCustomer.JPO,SimpleArea.AETP))
...
Services<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  write.csv(file="charts//Services_project_cur.csv",row.names = FALSE,na = "",
            engine_contracts %>% filter(SimpleArea=="Services")  %>%
              format_data_for_plot(fy_var="Fiscal_Year",y_var="Action_Obligation_Then_Year",color_var = "TopProject"#,
                                   # labels_and_colors=labels_and_colors
                                   )%>% arrange(Fiscal_Year) %>%
            pivot_wider(id_cols=c(TopProject),
...
aircraft<-topline_contracts %>% filter(PlatformPortfolio=="Aircraft")%>%
  mutate(type="Aircraft")


percent_contracts <- engine_contracts %>% 
  select(Fiscal_Year,dFYear, SubCustomer.JPO, SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type) %>%
  rbind(topline_contracts %>% filter(Fiscal_Year>=2000)%>%
          select(Fiscal_Year,dFYear, SubCustomer.JPO, SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type)) %>%
    rbind(aircraft %>% filter(Fiscal_Year>=2000)%>%
          select(Fiscal_Year,dFYear, SubCustomer.JPO, SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type)) %>%
  group_by(Fiscal_Year,dFYear, SubCustomer.JPO, SimpleArea.AETP, type) %>%
  dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
  pivot_wider(names_from=type, values_from=Action_Obligation_OMB23_GDP21) %>%
  group_by(Fiscal_Year,dFYear, SubCustomer.JPO, SimpleArea.AETP) %>%
...
(both<-gridExtra::grid.arrange(SimpleArea+facet_wrap(~SimpleArea.AETP)+labs(caption=NULL)+
                            ...
                            scale_x_date(labels = scales::label_date("'%y"),date_breaks = "5 years")+
                            theme(legend.position="right"),
                          pSA+scale_x_date("Fiscal Year",labels = scales::label_date("'%y"),date_breaks = "5 years"))
    )
...
write.csv(file="charts//pSimpleArea.csv",row.names = FALSE, na = "",
          pSA$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(SimpleArea.AETP,name),
...
pSubCustomer<-percent_contracts %>%
  group_by(Fiscal_Year,dFYear, SubCustomer.JPO) %>%
  dplyr::summarise(Engines = sum(Engines, na.rm = TRUE),
                   Aircraft = sum(Aircraft, na.rm = TRUE),
                   Topline = sum(Topline, na.rm = TRUE))%>%
  mutate(pTopline=Engines/Topline,
...
(
  pSC<-build_plot(
    data=pSubCustomer %>% filter(SubCustomer.JPO %in% c("Army","Navy","Air Force","DLA","F-35 JPO")),
    chart_geom = "Line Chart",
    share = FALSE,
    # labels_and_colors=labels_and_colors,
    # NA, #VAR.ncol
...
write.csv(file="charts//pSubCustomerJPO.csv",row.names = FALSE, na = "",
          pSC$data%>% #mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(SubCustomer.JPO,name),
                        names_from=Fiscal_Year,values_from=value))
```