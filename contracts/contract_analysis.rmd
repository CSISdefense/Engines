...
labels_and_colors<-prepare_labels_and_colors(engine_contracts %>% select(-PricingMechanism),
                           path="F:\\Users\\gsanders\\Documents\\Repositories\\Lookup-Tables\\style\\")
...
(
CustCompMultiSimpleArea<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year>=1991&
                                     SubCustomer.JPO %in% c("Army","Navy","Air Force","DLA","F-35 JPO")),
...
comparison_contracts <- engine_contracts %>%
  select(Fiscal_Year, SubCustomer, SimpleArea,SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type) %>%
  rbind(topline_contracts %>%
          select(Fiscal_Year, SubCustomer, SimpleArea, SimpleArea.AETP,Action_Obligation_OMB23_GDP21, type)) %>%
  group_by(Fiscal_Year, SubCustomer, SimpleArea,SimpleArea.AETP, type) %>%
  dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE))
...
percent_contracts <- engine_contracts %>% 
  select(Fiscal_Year, SubCustomer.JPO, SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type) %>%
  rbind(topline_contracts %>% filter(Fiscal_Year>=2000)%>%
          select(Fiscal_Year, SubCustomer.JPO, SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type)) %>%
    rbind(aircraft %>% filter(Fiscal_Year>=2000)%>%
          select(Fiscal_Year, SubCustomer.JPO, SimpleArea.AETP, Action_Obligation_OMB23_GDP21, type)) %>%
  group_by(Fiscal_Year, SubCustomer.JPO, SimpleArea.AETP, type) %>%
  dplyr::summarise(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21, na.rm = TRUE)) %>%
  pivot_wider(names_from=type, values_from=Action_Obligation_OMB23_GDP21) %>%
  group_by(Fiscal_Year, SubCustomer.JPO, SimpleArea.AETP) %>%
...
pSimpleArea.AETP<-percent_contracts %>%
  group_by(Fiscal_Year, SimpleArea.AETP) %>%
  dplyr::summarise(Engines = sum(Engines, na.rm = TRUE),
                   Aircraft = sum(Aircraft, na.rm = TRUE),
                   Topline = sum(Topline, na.rm = TRUE))%>%
...
ggsave600dpi(gridExtra::grid.arrange(pSA+labs(caption=NULL),pSA),file="charts/pSimpleArea.svg",width=6.5,height=2.5,size = 11,lineheight = 1.2)

ggsave(file="charts/SimpleAreaBoth.svg",
       gridExtra::grid.arrange(SimpleAreaWide+labs(caption=NULL)+
                          labs(y="Obligations\n(Constant 2021 $s)"),pSA),
...
pSubCustomer<-percent_contracts %>%
  group_by(Fiscal_Year, SubCustomer.JPO) %>%
  dplyr::summarise(Engines = sum(Engines, na.rm = TRUE),
                   Aircraft = sum(Aircraft, na.rm = TRUE),
                   Topline = sum(Topline, na.rm = TRUE))%>%
  mutate(pTopline=Engines/Topline,
