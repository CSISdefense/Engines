...
topplat<-engine_contracts %>% group_by (ProjectName,SimpleArea) %>%
...
engine_contracts<-left_join(engine_contracts,topplat %>% select(-Action_Obligation_OMB23_GDP21,Action_Obligation_2020),
          by=c("ProjectName","SimpleArea"))

engine_contracts$TopProject[is.na(engine_contracts$TopProject) & !is.na(engine_contracts$ProjectName)]<-
...
# )
# 
# ggsave600dpi("charts//psr_RnDPhase.png", RnDphase, 
...
# engine_contracts$Fiscal_Year
# 
# topplat<-engine_contracts %>% filter(SimpleArea=="Aircraft") %>% group_by (ProjectName) %>%
#   summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21),
#             Action_Obligation_2020=sum(ifelse(Fiscal_Year==2020,Action_Obligation_OMB23_GDP21,0))
#             )%>% mutate(rank_total=rank(desc(Action_Obligation_OMB23_GDP21)),
#                         rank_2020=rank(desc(Action_Obligation_2020)))
# topplat %>% arrange(desc(Action_Obligation_OMB23_GDP21))
# 
# engine_contracts$TopAircraftProject<-
#   ifelse(engine_contracts$ProjectName %in% topplat$ProjectName[topplat$rank_2020<=7 | topplat$rank_total<=7],
#          engine_contracts$ProjectName,NA)
# engine_contracts$TopAircraftProject[is.na(engine_contracts$TopAircraftProject) & !is.na(engine_contracts$ProjectName)]<-
#   "Other Labeled Project"
# summary(factor(engine_contracts$TopAircraftProject))
...
ggsave600dpi(gridExtra::grid.arrange(SimpleArea+labs(caption=NULL),pSA),file="charts/pSimpleArea.svg",width=6.5,height=2.5,size = 11,lineheight = 1.2)
