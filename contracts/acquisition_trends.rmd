...
main.text.size<-1
note.text.size<-1.40
if(!exists("engine_contracts"))
  # load(file="..//data//clean//unaggregated_FPDS_2020.Rda")
load(file="FPDS_chart_maker//unaggregated_FPDS.Rda")

# engine_contracts<-deflate(engine_contracts, "Action_Obligation_Then_Year",deflator_var="OMB23_GDP21")
...
column_key<-get_column_key(engine_contracts  %>% select(-PricingMechanism ))
...
ToplineCompMulti<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year>=1991&
...
ggsave600dpi(file="charts//EngineCompSubCust.png", ToplineCompMulti,  
...
write.csv(file="..//Output//topline_competition_cur.csv",row.names = FALSE, na = "",
          engine_contracts %>% group_by(Competition.sum,Fiscal_Year)%>%
...
UnlabeledComp<-build_plot(
  data=engine_contracts %>% filter(Competition.sum =="Unlabeled"),
  chart_geom = "Bar Chart",
  share = FALSE,
  # labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
#           engine_contracts %>% group_by(Shiny.VendorSize,Fiscal_Year)%>%
#             dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
...
engine_contracts %>% filter(Vehicle=="Unlabeled Indefinite Delivery Contract (IDC)" ) %>% group_by(ClassifyNumberOfOffers,No.Competition.sum)%>%
  summarise(n=length(Vehicle))


(
ToplineVehicle<-build_plot(
  data=engine_contracts %>% filter(!is.na(Vehicle.AwardTask)),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
## Topline Pricing
```{r ToplinePricing}
engine_contracts$PricingUCA.sumlong<-engine_contracts$PricingUCA.sum
levels(engine_contracts$PricingUCA.sumlong)<-list("Firm-Fixed-Price"="FFP",
...
summary(engine_contracts$ContractingSubCustomer)
def_data$
...
output_TY<-engine_contracts %>% group_by(SimpleArea,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
...
contract_merge<-engine_contracts %>%
...
contract_merge<-engine_contracts%>%filter(Fiscal_Year >=2015) %>%
  group_by(Fiscal_Year,
                         ProductServiceOrRnDarea,
                         SimpleArea)%>%
  dplyr::summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21))
...
(
Services<-build_plot(
  data=engine_contracts %>% filter(SimpleArea=="Services (Non-R&D)"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
SimpleVendor_dollar<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year<=2020 & !is.na(SimpleArea) & SimpleArea!="Unlabeled"),
  chart_geom = "Bar Chart",
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
  x_var="dFYear", #x_var
...
PSRComp_line<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year>=1991 & SimpleArea!="Unlabeled"),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//topline_subcustomer_current.csv",row.names = FALSE, na = "",
          engine_contracts %>% group_by(SubCustomer.sum,ContractingSubCustomer,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
...
```{r SubCustomerComp}
engine_contracts$SubCustomer.platform
(
...
output_TY<-engine_contracts %>% group_by(PlatformPortfolio,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
...
for(s in c("Army","Navy","Air Force")){
  ...
  (
    Platform<-build_plot(
      data=engine_contracts %>% filter(SubCustomer.sum==s),
      chart_geom = "Line Chart",
      share = FALSE,
      labels_and_colors=labels_and_colors,
      # NA, #VAR.ncol
...
# 
# write.csv(file="..//Output//topline_platform_current.csv",row.names = FALSE, na = "",
#           engine_contracts %>% group_by(PlatformPortfolio,Fiscal_Year)%>%
#             dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
#           pivot_wider(id_cols=c(PlatformPortfolio),
#                       names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%arrange(PlatformPortfolio))
# 
...
PlatformVendor_share<-build_plot(
  data=engine_contracts %>% filter(PlatformPortfolio!="Unlabeled" & Fiscal_Year<=2020) ,
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
VendorSizeVehicle<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year<=2020 & Fiscal_Year>2000 & Shiny.VendorSize!="Unlabeled"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
VendorSizeVehicleSumLine<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year<=2020 & Fiscal_Year>2000 & !is.na(Shiny.VendorSize)),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
(
VendorSizeVehicle<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year<=2020 & Fiscal_Year>2000 & !is.na(VendorSize_Intl)),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
(
VendorSizeVehicle<-build_plot(
  data=engine_contracts %>% filter(Fiscal_Year<=2020 & Fiscal_Year>2000),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
summary(engine_contracts$foreign_funding_description)

# undebug(build_plot)

(contract_identification <-  build_plot(data=engine_contracts ,
                            chart_geom="Bar Chart",
                            share=FALSE,
                            x_var="Fiscal_Year",
...
summary(engine_contracts$PlaceOfManufacture_Sum)
...
(contract_identification <-  build_plot(data=engine_contracts %>% filter(!is.na(PlaceIsForeign)),
                            chart_geom="Bar Chart",
                            share=FALSE,
                            x_var="Fiscal_Year",
                            y_var="Action_Obligation_OMB23_GDP21",
...
summary(engine_contracts$PlaceOfManufactureText)
engine_contracts$PlaceOfManufactureText20<-factor_wrap(engine_contracts$PlaceOfManufactureText)
# undebug(build_plot)
summary(factor(engine_contracts$PlaceOfManufactureText20))

write.csv(engine_contracts %>% group_by(PlaceOfManufactureText,PlaceIsForeign,VendorIsForeign,OriginIsForeign) %>%
...
engine_contracts$OriginManufactureVendor<-NA
# engine_contracts$OriginManufactureVendor[engine_contracts$PlaceOfManufacture_Sum=="Manufactured Outside U.S."]<-"Foreign Manufacture or Origin"
engine_contracts$OriginManufactureVendor[engine_contracts$OriginIsForeign==1]<-"Foreign Manufacture or Origin"
engine_contracts$OriginManufactureVendor[is.na(engine_contracts$OriginManufactureVendor)]<-
  engine_contracts$PlaceOfManufactureText[is.na(engine_contracts$OriginManufactureVendor)]
 ...
 # debug(factor_wrap)
 engine_contracts$OriginManufactureVendor20<-factor_wrap(engine_contracts$OriginManufactureVendor)
 
engine_contracts$Service<-factor(engine_contracts$SimpleArea)
levels(engine_contracts$Service)<-list("Products"="Products (All)",
                                "Services and R&D"=c("R&D", "Services (Non-R&D)"),
                                "Unlabeled"="Unlabeled")
 
engine_contracts$OriginManufactureVendor<-engine_contracts$PlaceOfManufacture_DoD

 engine_contracts$OriginManufactureVendor[is.na(engine_contracts$OriginManufactureVendor)&                                    
                                     (engine_contracts$Service=="Services and R&D" | 
                                        engine_contracts$PlaceOfManufactureText=="NOT A MANUFACTURED END PRODUCT") &
                                     (engine_contracts$VendorIsForeign==1 | engine_contracts$OriginIsForeign==1)]<-"Not MFG, Foreign Vendor or Origin"
 engine_contracts$OriginManufactureVendor[engine_contracts$OriginManufactureVendor=="U.S. Manufactured" &
                                    (engine_contracts$VendorIsForeign==1 | engine_contracts$OriginIsForeign==1)]<-"U.S. Manufactured, Foreign Vendor or Origin"

  engine_contracts %>% filter(OriginManufactureVendor=="Unlabeled Waiver" &
                         Service=="Services and R&D" )  %>% group_by(VendorIsForeign,OriginIsForeign) %>%
    summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21))
 
 
 engine_contracts %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
 ...
 # engine_contracts$OriginManufactureVendor[is.na(engine_contracts$OriginManufactureVendor)&
 #                                     engine_contracts$VendorIsForeign==1&engine_contracts$OriginIsForeign==0]<-"U.S. Origin Intl Vendor"
 # 
 # engine_contracts$OriginManufactureVendor[is.na(engine_contracts$OriginManufactureVendor)&
 #                                     engine_contracts$VendorIsForeign==0&engine_contracts$OriginIsForeign==1]<-"Intl Vendor U.S. Origin"
 #  
 # engine_contracts$OriginManufactureVendor[is.na(engine_contracts$OriginManufactureVendor)&
 #                                     engine_contracts$VendorIsForeign==1&engine_contracts$OriginIsForeign==1]<-"Intl Vendor Intl Origin"
 # 
 # engine_contracts$OriginManufactureVendor[engine_contracts$OriginManufactureVendor=="Unlabeled Waiver" &
 #                                     engine_contracts$Service=="Services and R&D" &
 #                                     engine_contracts$VendorIsForeign==0&engine_contracts$OriginIsForeign==1]<-"Intl Vendor U.S. Origin"
 #  
 # engine_contracts$OriginManufactureVendor[engine_contracts$OriginManufactureVendor=="Unlabeled Waiver" & 
 ...
 #  
  ...
  engine_contracts %>% filter(OriginManufactureVendor=="Unlabeled Waiver" &
   ...
   engine_contracts %>% filter(is.na(OriginManufactureVendor) &
                         Service=="Services and R&D" )  %>% group_by(VendorIsForeign,OriginIsForeign) %>%
    summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21))
...
(waiverdetail <-  build_plot(data=engine_contracts %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
...
##### Simple Waiver Type
```{r SimpleWaiver}
engine_contracts$PlaceOfManufacture_DoD09<-engine_contracts$PlaceOfManufacture_DoD
engine_contracts %>% filter(OriginManufactureVendor=="Unlabeled Waiver" &
                         Service=="Services and R&D" )  %>% group_by(VendorIsForeign,OriginIsForeign) %>%
    summarise(Action_Obligation_OMB23_GDP21=sum(Action_Obligation_OMB23_GDP21))
 ...
 engine_contracts$PlaceOfManufacture_DoD09[is.na(engine_contracts$PlaceOfManufacture_DoD09)&                                    
                                     (engine_contracts$Service=="Services and R&D" | 
                                        engine_contracts$PlaceOfManufactureText=="NOT A MANUFACTURED END PRODUCT") &
                                     (engine_contracts$VendorIsForeign==1 | engine_contracts$OriginIsForeign==1)&
                                    engine_contracts$PlaceIsForeign==1]<-"Not MFG, Foreign Place of Performance"
 engine_contracts$PlaceOfManufacture_DoD09[engine_contracts$PlaceOfManufacture_DoD09=="U.S. Manufactured" &
                                    (engine_contracts$VendorIsForeign==1 | engine_contracts$OriginIsForeign==1)]<-
 ...
 engine_contracts$PlaceOfManufacture_DoD09[is.na(engine_contracts$PlaceOfManufacture_DoD09)&                                    
                                     (engine_contracts$Service=="Services and R&D" | 
                                        engine_contracts$PlaceOfManufactureText=="NOT A MANUFACTURED END PRODUCT") &
                                     (engine_contracts$VendorIsForeign==1 | engine_contracts$OriginIsForeign==1)&
                                    engine_contracts$PlaceIsForeign==0]<-"Not MFG, U.S. Place of Performance"
  engine_contracts$PlaceOfManufacture_DoD09[engine_contracts$PlaceOfManufacture_DoD09=="Foreign MFG, Perfomed, or Content, Unlabeled Waiver"]<-NA
 
engine_contracts$PlaceOfManufacture_DoD09<-factor_wrap(engine_contracts$PlaceOfManufacture_DoD09,nwrap=45)
 
  
(waiversimple <-  build_plot(data=engine_contracts %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1|OriginIsForeign==1)  %>%
...
#### Waiverplace
```{r WaiverPlace}
  (waiverally <-  build_plot(data=engine_contracts %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1|OriginIsForeign==1) ,
                            chart_geom="Bar Chart",
...
(placemanufacture <-  build_plot(data=engine_contracts %>% filter(!is.na(PlaceIsForeign)),
                            chart_geom="Bar Chart",
                            share=FALSE,
                            x_var="Fiscal_Year",
                            y_var="Action_Obligation_OMB23_GDP21",
...
(vendormanufacture <-  build_plot(data=engine_contracts ,
                            chart_geom="Bar Chart",
                            share=FALSE,
                            x_var="Fiscal_Year",
                            y_var="Action_Obligation_OMB23_GDP21",
...
levels(factor(engine_contracts$PlaceStateRegion))
engine_contracts$PlaceStateRegion<-factor(engine_contracts$PlaceStateRegion)
engine_contracts$PlaceStateRegionSum<-factor(engine_contracts$PlaceStateRegion)
levels(engine_contracts$PlaceStateRegionSum)<-
...
summary(engine_contracts$PlaceStateRegionSum)

engine_contracts$PlaceOfManufactureText20<-factor_wrap(engine_contracts$PlaceOfManufactureText)
# undebug(build_plot)
summary(factor(engine_contracts$PlaceOfManufactureText20))
(placeregion <-  build_plot(data=engine_contracts %>% filter((PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
...
summary(engine_contracts$vendor)
engine_contracts$PlaceMutualAcquisition
(placeregion <-  build_plot(data=engine_contracts %>% filter((PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1|OriginIsForeign==1)&!is.na(PlaceStateRegion)
...
engine_contracts$PlaceAcquisitionCooperation
...
engine_contracts$place


(vendorregion <-  build_plot(data=engine_contracts %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1|OriginIsForeign==1) ,
                            chart_geom="Bar Chart",
...
(originregion <-  build_plot(data=engine_contracts %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1|OriginIsForeign==1) ,
                            chart_geom="Bar Chart",
...
(korea <-  build_plot(data=engine_contracts %>% filter(PlaceISOalpha3=="KOR" |
                                                  VendorISOalpha3=="KOR" |
                                                  OriginISOalpha3=="KOR"),
                            chart_geom="Bar Chart",
                            share=FALSE,
